...
def distributed_lock(r: redis.Redis, resource: str, ttl: int = 5):
    ...
    """
    key = f"lock:{resource}"
    # Tenta adquirir o lock com retry (espera ativa) — necessario em cenarios com processos concorrentes
    acquired = False
    ...
    while not acquired and time.time() < deadline:
        ...
        if not acquired:
            time.sleep(0.01)
    if not acquired:
        raise RuntimeError(f"Recurso '{resource}' em uso — timeout ao aguardar lock")
    try:
        yield
    finally:
